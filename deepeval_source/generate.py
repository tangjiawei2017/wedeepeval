...
class CustomOpenAI(DeepEvalBaseLLM):
    def __init__(
        ...
        self.call_count = 0
        self.cost_history = []  # List of (call_number, input_tokens, output_tokens, cost)
    ...
    def generate(self, prompt: str) -> str:
        try:
            ...
            if hasattr(completion, 'usage') and completion.usage:
                ...
                print(f"Response: {response}")
                print("Cost: Unable to calculate (no usage data)")
    ...
    def generate_with_cost(self, prompt: str) -> tuple[str, float]:
        """Generate response and return both response and cost"""
        try:
            messages = [{"role": "user", "content": prompt},{"role": "system", "content": "Please return a JSON object"}]
            ...
            error_msg = str(e)
            if "model_not_found" in error_msg or "does not exist" in error_msg:
                return f"Error: Model '{self.model_name}' not available. Please use one of the listed models.", 0.0
            else:
    ...
    def calculate_cost(self, input_tokens: int, output_tokens: int) -> float:
        ...
        self.cost_history.append((self.call_count, input_tokens, output_tokens, cost))
    ...
    def print_cost_summary(self):
        ...
        print(f"\n{'='*60}")
        print(f"💰 COST SUMMARY FOR {summary['model_name']}")
        print(f"{'='*60}")
        ...
        if len(self.cost_history) > 0:
            ...
            for call_num, input_tokens, output_tokens, cost in self.cost_history[-5:]:
                print(f"  Call #{call_num}: {input_tokens} in + {output_tokens} out = ${cost:.6f}")
        print(f"{'='*60}\n")
