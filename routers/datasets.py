...
async def process_document_generation(task_id: int, filename: str, content_bytes: bytes, suffix: str, num_questions: int):
    ...
    try:
        ...
        await asyncio.to_thread(
            lambda: open(temp_path, 'wb').write(content_bytes)
        )

        # 完全异步调用大模型，不等待返回结果
        logger.info(f"开始完全异步调用大模型生成文档数据...")
        ...
        async def background_document_generation():
            try:
                ...
                if not qa_items:
                    await asyncio.to_thread(task_manager.update_task_status, task_id, "failed", error_message="未生成任何问答对")
                    return
        ...
        # 启动后台任务，不等待结果
        asyncio.create_task(background_document_generation())
        
        # 任务已在后台启动，直接返回
        logger.info(f"文档任务 {task_id} 已在后台启动，不等待结果")
    except Exception as e:
        import traceback
        error_details = traceback.format_exc()
        logger.error(f"文档任务 {task_id} 失败: {str(e)}\n错误详情: {error_details}")
...
async def process_context_generation(task_id: int, payload: FromContextRequest):
    ...
    try:
        ...
        # 完全异步调用大模型，不等待返回结果
        logger.info(f"开始完全异步调用大模型生成上下文数据...")
        ...
        async def background_context_generation():
            try:
                ...
                # 检查DeepEval生成结果
                if not qa_items:
                    await asyncio.to_thread(task_manager.update_task_status, task_id, "failed", error_message="DeepEval没有成功生成任何问答对")
                    return
        ...
        # 启动后台任务，不等待结果
        asyncio.create_task(background_context_generation())
        
        # 任务已在后台启动，直接返回
        logger.info(f"上下文任务 {task_id} 已在后台启动，不等待结果")
    except Exception as e:
        import traceback
        error_details = traceback.format_exc()
        logger.error(f"上下文任务 {task_id} 失败: {str(e)}\n错误详情: {error_details}")
        await asyncio.to_thread(task_manager.update_task_status, task_id, "failed", error_message=str(e))
    ...
    try:
        ...
        async def background_generation():
            try:
                ...
                if not qa_items:
                    await asyncio.to_thread(task_manager.update_task_status, task_id, "failed", error_message="没有生成任何问答对")
                    return
        ...
        # 任务已在后台启动，直接返回
        logger.info(f"主题任务 {task_id} 已在后台启动，不等待结果")
    except Exception as e:
        import traceback
        error_details = traceback.format_exc()
        logger.error(f"主题任务 {task_id} 失败: {str(e)}\n错误详情: {error_details}")
...
async def process_augment_generation(task_id: int, contexts: List[str], target_num: int):
    ...
    try:
        ...
        # 完全异步调用大模型，不等待返回结果
        logger.info(f"开始完全异步调用大模型生成扩写数据...")
        ...
        async def background_augment_generation():
            try:
                ...
                if not qa_items:
                    await asyncio.to_thread(task_manager.update_task_status, task_id, "failed", error_message="未生成任何扩写数据")
                    return
        ...
        # 启动后台任务，不等待结果
        asyncio.create_task(background_augment_generation())
        
        # 任务已在后台启动，直接返回
        logger.info(f"扩写任务 {task_id} 已在后台启动，不等待结果")
    except Exception as e:
        import traceback
        error_details = traceback.format_exc()
        logger.error(f"扩写任务 {task_id} 失败: {str(e)}\n错误详情: {error_details}")
...
async def download_template():
    ...
    # 异步检查文件是否存在
    if not await asyncio.to_thread(os.path.exists, template_file_path):
